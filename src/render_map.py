...
def render_map(vehicle: Vehicle, route:list[tuple[int]], outfile:str):
    ...
    for i, (address, coord) in enumerate(route[:-1]):
        ...
        folium.PolyLine(
            ...
            color="blue",
            popup=f'Edge {i+1}<br><a href="https://www.google.com/maps/dir/?api=1&origin={coord_1[0]},{coord_1[1]}&destination={coord_2[0]},{coord_2[1]}&travelmode={maps_type}" target="_blank">Route to next node</a>',
            tooltip=f"Edge {i+1}",
        ).add_to(mymap)
        folium.Marker(
            location=midpoint,
